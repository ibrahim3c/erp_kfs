    ...
    public sealed class LeadershipPosition : Entity
    {
            ...
            string description)
        {
            if (orgUnitId == Guid.Empty)
                return Result<LeadershipPosition>.Failure(EmployeeErrors.OrgUnitIdEmpty);

            if (jobTitleId == Guid.Empty)
                return Result<LeadershipPosition>.Failure(EmployeeErrors.JobTitleIdEmpty);
        ...
        {
            if (!IsActive)
                return Result.Failure(EmployeeErrors.LeadershipAlreadyInactive);
        ...
        {
            if (IsActive)
                return Result.Failure(EmployeeErrors.LeadershipAlreadyActive);
