    ...
    public class FunctionalGroup : Entity
    {
        ...
        public static Result<FunctionalGroup> Create(Guid qualitativeGroupId, string code, string name, string description)
        {
            if (qualitativeGroupId == Guid.Empty)
                return Result<FunctionalGroup>.Failure(OrganizationErrors.QualitativeGroupIdEmpty);

            if (string.IsNullOrWhiteSpace(code))
                return Result<FunctionalGroup>.Failure(OrganizationErrors.CodeEmpty);

            if (string.IsNullOrWhiteSpace(name))
                return Result<FunctionalGroup>.Failure(OrganizationErrors.NameEmpty);
